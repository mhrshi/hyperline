<template>
  <v-app>
    <v-toolbar prominent>
      <v-toolbar-title class="text-xs-center headline font-monospace game-title"
        >hyperline</v-toolbar-title
      >
      <v-spacer></v-spacer>
      <v-toolbar-items>
        <v-menu v-slot:activator="{ on }" transition="slide-y-transition" bottom offset-y>
          <v-btn v-on="on" text slot="activator">level</v-btn>
          <v-list>
            <v-list-tile
        ...
        <v-menu v-slot:activator="{ on }" transition="slide-y-transition" bottom offset-y>
          <v-btn v-on="on" text slot="activator">variant</v-btn>
          <v-list>
            <v-list-tile
    ...
    <v-container class="pa-0" fill-height fluid>
      <v-layout class="wrapper">
        <v-flex class="game" justify-center align-center>
          <div class="pa-5 grid" :class="`grid-size-${currentLevel}`">
            <div class="grid-box" v-for="(value, i) in gridArray" :key="i" @click="markSpace(i)">
              <v-img v-if="value" :src="require(`../assets/p${value}.svg`)" max-width="35%" contain>
              </v-img>
            </div>
          </div>
        </v-flex>
        <v-flex :class="`updates ${resultColor} lighten-5`">
          <v-layout class="py-5 mx-5" column fill-height>
            <div class="mb-5 versus">
              ...
              <v-img class="mx-4" :src="require('../assets/versus.svg')" max-width="50px" contain>
              </v-img>
...
export default {
  ...
  methods: {
    ...
    markSpace(index) {
      ...
      this.$store.state.socket.emit("turnPlayed", { index, gridArray: this.gridArray });
    },
